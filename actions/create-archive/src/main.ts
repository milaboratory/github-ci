...
async function getAllFilesFromDir(dir: string): Promise<string[]> {
    ...
    for (const directory of directories) {
        files = files.concat(await getAllFilesFromDir(path.join(dir, directory.name)));
    }
    return files;
}
...
async function main(): Promise<void> {
    ...
    const rawSourceFiles = core.getInput('source-files').split('\n').map(item => item.trim());
    ...
    const filteredFiles = allFiles.filter(file =>
        matchFilename(file, includePatterns) && !matchFilename(file, excludePatterns)
    );
