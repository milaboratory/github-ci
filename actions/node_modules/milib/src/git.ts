 ...
 */
export async function latestVersionTag(): Promise<string> {
  const tagsListStr = await tag({
    list: true,
  ...
  for (const tagName of tagsList) {
    if (tagName.startsWith("v")) {
      tagsMap[tagName.slice(1)] = tagName;
    } else {
      tagsMap[tagName] = tagName;
    }
  }

  const versionsList = Object.keys(tagsMap);
  versionsList.sort((a, b) => a.localeCompare(b));

  const latestVersion: string = versionsList[versionsList.length - 1];

  return tagsMap[latestVersion];
}
